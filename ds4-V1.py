...
#ds4.py AND Tank.py AND ds4drv should all be placed inside /home/pi
#Running Tank.py will first set up the drives by calling ds4drv and then call ds4.py
#Thus these 3 files are the only ones needed for the bluetooth pi and you only need to run Tank.py on it which will automatically set everything up
...
#------------DEFINING KEYBOARD CAPTURE FUNCTION-------------------#------UPDATE GETCH AND IMPORT IT IF U DONT WANT TO DEF THIS-------#
def getch():
    fd = sys.stdin.fileno()
...
#------Call this function by defining a separate variable like keyboard=getch()-------------------#
...
while True:
    ...
    #--------------KEYBOARD KEYS SENT TO BROKER---------------------#
    keyboard = getch()
    if(keyboard == "w"):
        print"KEY W PRESSED"
        mqttc.publish("ds4",'w',0)
    if(keyboard == "s"):
        print"KEY S PRESSED"
        mqttc.publish("ds4",'s',0)
    if(keyboard == "a"):
        print"KEY A PRESSED"
        mqttc.publish("ds4",'a',0)
    if(keyboard == "d"):
        print"KEY D PRESSED"
        mqttc.publish("ds4",'d',0)
    keyboard = ""  #The keyboard variable will be set to blank, ready to save the next key that is pressed
